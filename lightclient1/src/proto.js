...
(function (global, factory) {
  ...
  var $root =
    $protobuf.roots.machinelearning || ($protobuf.roots.machinelearning = {});
        ...
        switch (tag >>> 3) {
          ...
          case 3:
            ...
            if ((tag & 7) === 2) {
              ...
              while (reader.pos < end2) message.weights.push(reader.float());
            } else message.weights.push(reader.float());
            break;
          default:
    ...
    Model.toObject = function toObject(message, options) {
      ...
      if (message.weights && message.weights.length) {
        ...
        for (var j = 0; j < message.weights.length; ++j)
          object.weights[j] =
            options.json && !isFinite(message.weights[j])
              ? String(message.weights[j])
              : message.weights[j];
      }
      return object;
    };
  ...
  $root.TxShareUpdates = (function () {
     ...
     * @instance
     */
    TxShareUpdates.prototype.seed = $util.Long
    ...
    TxShareUpdates.decode = function decode(reader, length) {
      if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
      var end = length === undefined ? reader.len : reader.pos + length,
        message = new $root.TxShareUpdates();
      while (reader.pos < end) {
        var tag = reader.uint32();
        switch (tag >>> 3) {
          case 1:
            ...
            if ((tag & 7) === 2) {
              var end2 = reader.uint32() + reader.pos;
              while (reader.pos < end2) message.gradients.push(reader.float());
            } else message.gradients.push(reader.float());
            break;
          case 2:
    ...
    TxShareUpdates.verify = function verify(message) {
      ...
      if (message.seed != null && message.hasOwnProperty('seed'))
        if (
          !$util.isInteger(message.seed) &&
          !(
            message.seed &&
            $util.isInteger(message.seed.low) &&
            $util.isInteger(message.seed.high)
          )
        )
    ...
    TxShareUpdates.fromObject = function fromObject(object) {
      if (object instanceof $root.TxShareUpdates) return object;
      ...
      if (object.seed != null)
        ...
        else if (typeof object.seed === 'number') message.seed = object.seed;
        else if (typeof object.seed === 'object')
          message.seed = new $util.LongBits(
            object.seed.low >>> 0,
            object.seed.high >>> 0
          ).toNumber(true);
      return message;
    ...
    TxShareUpdates.toObject = function toObject(message, options) {
      if (!options) options = {};
      var object = {};
      if (options.arrays || options.defaults) object.gradients = [];
      if (options.defaults)
        if ($util.Long) {
      ...
      if (message.gradients && message.gradients.length) {
        ...
        for (var j = 0; j < message.gradients.length; ++j)
          object.gradients[j] =
            options.json && !isFinite(message.gradients[j])
              ? String(message.gradients[j])
              : message.gradients[j];
      }
      if (message.seed != null && message.hasOwnProperty('seed'))
        if (typeof message.seed === 'number')
          object.seed =
            options.longs === String ? String(message.seed) : message.seed;
        else
          object.seed =
            options.longs === String
              ? $util.Long.prototype.toString.call(message.seed)
              : options.longs === Number
              ? new $util.LongBits(
                  message.seed.low >>> 0,
                  message.seed.high >>> 0
